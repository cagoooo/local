...
|------|------|------|
| 版本 | **v2.6** | |
| 核心檔案 | `index.html` 172.3 KB | 單檔 HTML,含所有 CSS + JS |
...
### 🧪 v2.6 · 實戰驗證後的應對優化 · 5 項 · 2026-04-21

**契機**:真實案例測試配對率只有 19.4%,發現多項可改進之處。

| # | 項目 | 說明 |
|---|------|------|
| 40 | **班級格式 301 支援** | `normalizeClass()` 新增三位數格式偵測(301→3年1班、501→5年1班、609→6年9班)以及國字年級(三年1班、五年三班)· 23 個單元測試全過 |
| 41 | **低配對率強制警示 modal** | 配對率 < 50% 時彈出 modal,列出「未配對分布 by 舊班級」+ 可能原因 + 重新上傳按鈕 · `sessionStorage` 防重複提示 |
| 42 | **匯出前二次確認** | 配對率 < 80% 時 `confirm()` 提示「仍匯出 / 回去檢查」· 兩個匯出函式都包覆 |
| 43 | **匯出檔健康度指標頁首** | 矩陣匯出檔前加「📊 配對健康度指標」區塊(5-6 列表格 + 健康度評價 + 建議) · 評價分 4 級:優秀/良好/待確認/偏低 |
| 44 | **未配對區依舊班級分組** | 原本平面列表改為「── 2年1班 (21 位) · 原任教師 XXX」分組,每班獨立表格,方便逐班聯絡核對 |

**累計 44 項功能已完成,第一、二、六波全數打勾;本版特色是「從真實案例反推優化」,符合 dogfooding 精神。**
...
已完成 **44 項**,還有 **42 項**藍圖。每完成一項回來打 ✅,年底再回頭看會很有成就感。
...
| **v2.6** | **2026-04-21** | **44** | **實戰**(301 格式 / 低配對警示 / 健康度指標 / 分組未配對) |
...
| 版本 | 預計內容 | 預計時程 |
|------|---------|---------|
| **v2.7** | PWA Service Worker · 讓瀏覽器可「安裝到桌面」當原生 APP | 2-3 小時 |
| **v2.8** | 手動編輯單筆 + CSV 剪貼簿貼上 + URL 參數分享 | 一個週末 |
...
依您自己的節奏,不用趕。每個版本發佈後記得打 git tag(`git tag -a v2.6 -m "..."` 然後 `git push --tags`)。
