...
| 類別 | 能做到什麼 |
|------|-----------|
| **資料匯入** | 三種排版通吃:「一頁多班、多語別」矩陣格式(學校慣用)、校務系統匯出的「一列一位學生」表格、班級標題式名冊;檔案格式支援 `.xlsx` `.xls` `.xlsm` `.xlsb` `.ods` `.csv`(自動辨識 UTF-8 / Big5);彈性欄位名(姓名/學生姓名、班級/新班級、座號/新座號、選擇語別/本土語言);**可一次選取多個檔案**(同時處理三年級與五年級) |
...
點擊或拖曳上傳 `xx年級_未分班.xlsx`。**兩種排版都讀得到**:

| 排版 | 長相 | 常見來源 |
|------|------|---------|
| **矩陣式** | 一頁多班,每班有「班級:／語別:／編號」標頭,語別分欄並排 | 校內慣用的本土語名單 |
| **一列一生** | 一列一位學生,欄位如「姓名 / 選擇語別 / 班級 / 座號」 | 校務系統直接匯出 |
| **班級標題式** | 「3年1班」當標題,底下接座號/姓名兩欄(語別留空) | 各班回傳的簡易名冊 |

系統會自動:

- 解析每個班級(3年1班 ~ 3年5班 / 5年1班 ~ 5年5班)
- 辨識語別 — 不論寫「閩語/客語/族語」這種大類,或直接寫方言名(「四縣腔」「賽考利克泰雅語」「郡群布農語」「緬甸語」「臺灣手語」),都會歸到正確類別
- 擷取每位學生的舊班級、舊座號、姓名、選修語別、族別
- 班級只寫在該班第一列(其餘留白)時,自動往下沿用

成功後,會顯示統計小卡(學生總數、班級數、語別數)。

### Step 2:上傳新編班名冊

點擊或拖曳上傳學校給的新編班名冊。只要檔案包含「姓名」與「班級」兩欄即可(座號可留空,欄位名可以有變體,見下一節)。
...
姓名與班級是必要的;**座號可以整欄不給**(匯出時該欄留白)。

### 支援的檔案格式

| 副檔名 | 說明 |
|--------|------|
| `.xlsx` `.xlsm` `.xlsb` `.xls` | Excel 各版本 |
| `.ods` | LibreOffice / OpenOffice |
| `.csv` `.tsv` `.txt` | 純文字表格,**自動辨識 UTF-8 / Big5 編碼**(校務系統匯出的 Big5 CSV 不必先轉檔) |
...
|------|---------------|
| 姓名 | 姓名、學生姓名、名字 |
| 班級 | 新班級、原班級、舊班級、班級、班別、年班 |
| 座號 | 新座號、舊座號、座號、號碼(可省略) |
| 語別(舊名單) | 選擇語別、選修語別、修習語別、語別、本土語言、語言別 |
| 族別(選填) | 族別、族群、身分別,或寫在姓名括號內 |
| 導師(選填) | 導師、班導、教師、老師 |
...
│
├── test-formats.js         # 檔案格式相容性測試(直接抽 index.html 的函式來跑)
...
- 確認副檔名在支援清單內(`.xlsx` `.xls` `.xlsm` `.xlsb` `.ods` `.csv` `.tsv` `.txt`)
- 確認檔案內有「班級:」「語別:」「編號」等標籤
...
本土語分班配對系統 v2.1 · 小學升級分班後自動更新選修名單 · 單檔 HTML · 深色模式 · 模糊比對 · 100% 本機運算

- 專案定位：校務／行政流程數位化專案
- Repository：`cagoooo/local`
- 可見性：公開
- 主要技術：HTML
- 線上入口：[https://cagoooo.github.io/local/](https://cagoooo.github.io/local/)
